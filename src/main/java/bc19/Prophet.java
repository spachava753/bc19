...
public class Prophet extends RobotType {
    ...
    public Action takeTurn() {
        ...
        if (enemyRobot != null) {
            ...
            Log.d("ATTACK RADIUS ARRAY " + robot.SPECS.UNITS[robot.SPECS.PROPHET].ATTACK_RADIUS);
            Log.d("ATTACK RADIUS " + robot.SPECS.UNITS[robot.SPECS.PROPHET].ATTACK_RADIUS[1]);
            Log.d("ATTACK RADIUS SQRT " + Math.sqrt(robot.SPECS.UNITS[robot.SPECS.PROPHET].ATTACK_RADIUS[1]));
            if (distToEnemy >= Math.sqrt(robot.SPECS.UNITS[robot.SPECS.PROPHET].ATTACK_RADIUS[1])) {
                ...
                if (Math.abs(dx) > Math.sqrt(robot.SPECS.UNITS[robot.SPECS.PROPHET].SPEED)) {
                    dx = (int) (Math.floor(Math.sqrt(robot.SPECS.UNITS[robot.SPECS.PROPHET].SPEED)) * Math.signum(dx));
                }

                if (Math.abs(dy) > Math.sqrt(robot.SPECS.UNITS[robot.SPECS.PROPHET].SPEED)) {
                    dy = (int) (Math.floor(Math.sqrt(robot.SPECS.UNITS[robot.SPECS.PROPHET].SPEED)) * Math.signum(dy));
                }
